package com.compte.declaccise.web.rest;

import com.compte.declaccise.config.KafkaProperties;
...
class DeclacciseKafkaResourceIT {
    ...
    void setup() {
        ...
        DeclacciseKafkaResource kafkaResource = new DeclacciseKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/declaccise-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/declaccise-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
